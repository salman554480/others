...
<body>
    ...
    document.getElementById('uploadButton').addEventListener('click', async function() {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            try {
                const response = await fetch('upload.php', { // Change 'upload.php' to your PHP handling file
                    method: 'POST',
                    body: formData
                });

                const result = await response.text();
                document.getElementById('uploadStatus').innerHTML += `Response for ${files[i].name}: ${result}<hr>`;
            } catch (error) {
